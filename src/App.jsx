...
function App() {
  ...
  const [availableCoins, setAvailableCoins] = useState(6000000);
      ...
      <div className="flex justify-between items-center w-300 mx-auto mt-7">
        ...
        <div className="flex py-3 ">
          <button
            ...
            className={`btn py-2 px-4  text-black rounded-l-2xl border-r-0 ${toggle === true ? "bg-[#E7FE29]" : "bg-white"}`}
          >
            Available
          </button>
          <button
            onClick={() => setToggle(false)}
            className={`btn py-2 px-4  text-black rounded-r-2xl border-r-0 ${toggle === false ? "bg-[#E7FE29]" : "bg-white"}`}
          >
            Selected <span>({`${buyingPlayer.length}`})</span>
          </button>
        </div>
      </div>
      ...
      {toggle === true ? (
        ...
        >
          <AvailablePlayers
            ...
            setBuyingPlayer={setBuyingPlayer}
          />
        </Suspense>
      ) : (
        <SelectedPlayers
